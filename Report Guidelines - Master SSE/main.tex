...
\noindent Falls among elderly individuals are a major cause of injury, hospitalization, and death, posing a serious burden on healthcare systems worldwide[1]. Wearable sensors, particularly smartwatches, are emerging as an effective platform for continuous fall detection due to their accessibility, portability, and integration with smartphones[2]. Smartwatches typically incorporate Inertial Measurement Units (IMUs) that include accelerometers and gyroscopes capable of capturing fine-grained motion data in real time[3].

However, fall detection remains challenging due to the diversity of Activities of Daily Living (ADLs) and the transient nature of fall events, which often resemble benign motions such as sitting or jumping[4]. 

Recent research has proposed a range of deep learning-based models that aim to improve detection accuracy by learning robust motion patterns from raw sensor data, including CNNs, Transformers, and hybrid models[5][6]. In addition to accuracy, energy efficiency and real-time inference are key requirements for embedded AI deployment on wearables, motivating the use of quantization, pruning, and knowledge distillation[7][8].

This study presents a two-stage fall detection system optimized for deployment on smartwatches using 6-axis IMU data, combining lightweight candidate filtering with high-precision validation, and employing compression techniques to maintain high accuracy with low latency.
...
\noindent 1. Sensor Principles and Data Acquisition

Smartwatches typically include 6-axis Inertial Measurement Units (IMUs)—three-axis accelerometers (measuring linear acceleration: $A_x$, $A_y$, $A_z$) and three-axis gyroscopes (measuring angular velocity: $G_x$, $G_y$, $G_z$). These sensors continuously track the motion and orientation of the wearer's wrist.

Falls are characterized by:

•	A sudden increase in total acceleration (commonly seen as a spike in Signal Magnitude Vector, or SMV).

•	A significant angular shift or tilt (detected through gyroscope data and derived orientation).

•	A brief inactivity phase (after impact) that is atypical of most ADLs.

These signals are non-stationary (i.e., their statistical properties change over time), making traditional signal processing methods less effective. Therefore, recent research explores time-frequency transforms and deep learning approaches that capture both the transient and sequential nature of falls.
